#!/usr/bin/perl -w
###############################################################################
# $Id: utils.pm 1075500 2011-02-28 19:30:55Z arkurth $
###############################################################################
...
our ($EC2_ACCESS_KEY_ID, $EC2_SECRET_KEY, $REMOTE_VCL_USER, $REMOTE_VCL_PASS, $TMRK_USER, $TMRK_PASS);
...
=head2 update_computer_notes

 Parameters  : $computer_id, $notes, $log
 Returns     : 1 success 0 failure
 Description : update computer notes

=cut

sub update_computer_notes {
	my ($computer_id, $notes, $log) = @_;

	my ($package, $filename, $line, $sub) = caller(0);

	notify($ERRORS{'WARNING'}, $log, "computer id is not defined") unless (defined($computer_id));
	notify($ERRORS{'WARNING'}, $log, "notes is not defined")   unless (defined($notes));
	return 0 unless (defined $computer_id && defined $notes);
	...
	if (database_execute($update_statement)) {
		# Update successful
		notify($ERRORS{'OK'}, $LOGFILE, "computer $computer_id note updated to: $notes");
		return 1;
	}
	else {
		notify($ERRORS{'CRITICAL'}, 0, "unable to update notes for computer $computer_id");
		return 0;
	}
} ## end sub update_computer_notes
