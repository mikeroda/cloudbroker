...
sub reserve {
    ...
    my ($exit_status, $output) = run_ssh_command($computer_shortname, $management_node_keys, $command, $user);
   
    $command = "sudo cp ~$user/.ssh/authorized_keys /root/.ssh/";
    ($exit_status, $output) = run_ssh_command($computer_shortname, $management_node_keys, $command, $user);
    if ($exit_status != 0) {
